...
# The ONLYOFFICE value only lets settings load for collectstatic; runtime still requires the injected secret.
RUN DJANGO_DEBUG=0 DJANGO_SECRET_KEY=build DATABASE_URL=sqlite:///:memory: ONLYOFFICE_JWT_SECRET=build-only-not-a-runtime-secret python manage.py collectstatic --noinput
EXPOSE 8000
